...
function App() {
  ...
  const handleTimepointClick = (timepointId) => {
    ...
    fetch(`${API_BASE_URL}/component/components/${timepointId}/summary`)
      .then((res) => {
        if (!res.ok) {
          if (res.status === 404) {
            throw new Error(
              "Timepoint not found. The requested timepoint data is unavailable.",
            );
          }
          throw new Error(
            "Failed to fetch timepoint details. Please try again later.",
          );
        }
        return res.json();
      })
        ...
        <Box sx={{ my: 4 }}>
          ...
          >
            <TabList className="reactTabs__tabList">
              ...
              <Tab className="reactTabs__tab" disabled={!selectedTimepoint}>
                Components
              </Tab>
              <Tab className="reactTabs__tab" disabled={!selectedComponent}>
                Statistics
              </Tab>
            </TabList>
              ...
              <Grid container spacing={3}>
                              ...
                              <TableRow key={timepoint.id}>
                                ...
                                <TableCell>
                                  <Button
                                    ...
                                    color="primary"
                                    onClick={() =>
                                      handleTimepointClick(timepoint.id)
                                    }
                                  >
                                    Select
                                  </Button>
                ...
                <Grid item xs={12} md={6}>
                  <Paper elevation={3} sx={{ p: 3 }}>
                    ...
                    <Typography paragraph>
                      Welcome to the DMR Analysis Dashboard. This tool helps you
                      analyze DNA methylation regions and their relationships.
                    </Typography>
                    <Typography paragraph>
                      Select a timepoint from the table above to begin your
                      analysis.
                    </Typography>
                  </Paper>
                </Grid>
                ...
                <Grid item xs={12} md={6}>
                  <Paper elevation={3} sx={{ p: 3 }}>
                    ...
                    <Typography>No recent analysis sessions found.</Typography>
                  </Paper>
                </Grid>
              </Grid>
            </TabPanel>
                  ...
                  <Grid item xs={12}>
                    <ComponentsView
                      ...
                      onSelectComponent={(componentId) => {
                        // Add debug logging
                        console.log(
                          `Selected component ${componentId} from timepoint ${selectedTimepoint}`,
                        );
                        ...
                        if (selectedTimepoint) {
                          ...
                          console.error(
                            "No timepoint selected when choosing component",
                          );
                ...
                <Grid item xs={12}>
                  <Box>
                    <Typography variant="h5" gutterBottom>
                      Timepoint{" "}
                      {timepoints.find((t) => t.id === selectedTimepoint)?.name}{" "}
