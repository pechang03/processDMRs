...
def process_data():
    ...
    try:
        ...
        stats = {
            ...
            'components_with_bicliques': len([comp for comp in component_data if comp['bicliques']]),
...
def process_bicliques(bipartite_graph, df):
    ...
    return read_bicliques_file("./data/bipartite_graph_output.txt.biclusters", max_dmr_id, bipartite_graph)
...
def process_components(bipartite_graph, bicliques_result):
    ...
    for idx, component in enumerate(components):
        ...
        component_bicliques = [
            ...
            if any(node in component for node in dmr_nodes) and (len(dmr_nodes) > 1 or len(gene_nodes) > 1)
        ]
...
def create_metadata(df, gene_id_mapping):
    ...
    dmr_metadata = {
        f"DMR_{row['DMR_No.']}": {'area': row['Area_Stat'] if 'Area_Stat' in df.columns else 'N/A'}
        for _, row in df.iterrows()
    }

    gene_metadata = {
        gene: {'description': df[df['Gene_Symbol_Nearby'] == gene].iloc[0]['Gene_Description'] if len(df[df['Gene_Symbol_Nearby'] == gene]) > 0 else 'N/A'}
