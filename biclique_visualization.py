...
from graph_layout import (
    ...
    position_single_biclique,
    position_nodes_evenly,
)
...
def create_hub_traces(
    ...
    return [
        go.Scatter(
            ...
            mode="markers+text",
            marker=dict(
                size=15, color="gold", symbol="star", line=dict(color="orange", width=2)
            ),
            text=hub_text,
            textposition="middle left",
            hoverinfo="text",
...
def get_node_colors(
    bicliques: List[Tuple[Set[int], Set[int]]], biclique_colors: List[str]
) -> Dict[int, str]:
...
def separate_node_data(
    ...
    return (dmr_x, dmr_y, dmr_colors, dmr_text), (
        gene_x,
        gene_y,
        gene_colors,
        gene_text,
...
def create_gene_trace(x, y, colors, text) -> go.Scatter:
    ...
    return go.Scatter(
        ...
        mode="markers+text",
        marker=dict(
            size=10, color=colors, symbol="diamond", line=dict(color="black", width=1)
        ),
        text=text,
        textposition="middle right",
        hoverinfo="text",
