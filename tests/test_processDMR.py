...
class TestBipartiteGraph(unittest.TestCase):
    def setUp(self):
        ...
        self.df_home1["Processed_Enhancer_Info"] = self.df_home1[
            "ENCODE_Enhancer_Interaction(BingRen_Lab)"
        ].apply(process_enhancer_info)
        # Check if the node is a DMR and assert it has adjacent edges
        for dmr in self.bipartite_graph.nodes():
            if (
                self.bipartite_graph.nodes[dmr].get("bipartite") == 0
            ):  # Check if it's a DMR
                self.assertGreater(
                    len(list(self.bipartite_graph.adj[dmr])),
                    0,
                    f"{dmr} has no adjacent edges.",
                )
    ...
    def test_empty_dataframe(self):
        ...
        self.assertEqual(
            ...
            0,
            "Graph should have no nodes for an empty DataFrame.",
        )
    ...
    def test_single_node_graph(self):
        ...
        self.assertEqual(
            len(single_node_graph.nodes()),
            1,
            "Graph should have one node for a single-node DataFrame.",
        )
    ...
    def test_multiple_dmrs(self):
        ...
        self.assertEqual(
            len(graph.nodes()),
            11,
            "Graph should have 11 nodes (3 DMRs + 8 unique genes).",
        )
    ...
    def test_random_bipartite_graphs(self):
        ...
        for _ in range(10):
            ...
            data = {
                "DMR_No.": [
                    i for i in range(1, num_dmrs + 1) for _ in range(num_genes)
                ],
                "Gene_Symbol_Nearby": [
                    f"Gene{j}" for _ in range(num_dmrs) for j in range(num_genes)
                ],
                "ENCODE_Enhancer_Interaction(BingRen_Lab)": [None]
                * (num_dmrs * num_genes),
            }
            df = pd.DataFrame(data)
            df["Processed_Enhancer_Info"] = df["Gene_Symbol_Nearby"].apply(
            ...
            self.assertEqual(
                len(graph.nodes()),
                num_dmrs + num_genes,
                f"Graph should have {num_dmrs + num_genes} nodes.",
            )
            self.assertEqual(
                len(graph.edges()),
                num_dmrs * num_genes,
                f"Graph should have {num_dmrs * num_genes} edges.",
            )
    ...
    def test_sparse_and_dense_graphs(self):
        ...
        sparse_df["Processed_Enhancer_Info"] = sparse_df["Gene_Symbol_Nearby"].apply(
            lambda x: list(set(x.split(";"))) if x else []
        )
        sparse_graph = create_bipartite_graph(sparse_df)
        self.assertEqual(
            len(sparse_graph.nodes()),
            6,
            "Sparse graph should have 6 nodes (3 DMRs + 3 genes).",
        )
        self.assertEqual(
            len(sparse_graph.edges()), 3, "Sparse graph should have 3 edges."
        )
        ...
        self.assertEqual(
            len(dense_graph.nodes()),
            6,
            "Dense graph should have 6 nodes (3 DMRs + 3 genes).",
        )
        self.assertEqual(
            len(dense_graph.edges()),
            9,
            "Dense graph should have 9 edges (3 DMRs * 3 genes).",
        )
    ...
    def test_degree_one_genes(self):
        ...
        for node in graph.nodes():
            if (
                graph.nodes[node]["bipartite"] == 1  # is a gene
    ...
    def test_complete_bipartite_graphs(self):
        ...
        df_k23["Processed_Enhancer_Info"] = df_k23["Gene_Symbol_Nearby"].apply(
            lambda x: list(set(x.split(";"))) if x else []
        )
        ...
        data_k37 = {
            ...
            "Gene_Symbol_Nearby": genes_k37
            * 3,  # Each gene connected to all three DMRs
            "ENCODE_Enhancer_Interaction(BingRen_Lab)": [None] * 21,
        }
        df_k37 = pd.DataFrame(data_k37)
        df_k37["Processed_Enhancer_Info"] = df_k37["Gene_Symbol_Nearby"].apply(
            lambda x: list(set(x.split(";"))) if x else []
        )
        ...
        self.assertEqual(
            len(graph_k37.nodes()),
            10,
            "K_{3,7} should have 10 nodes (3 DMRs + 7 genes).",
        )
        self.assertEqual(
            len(graph_k37.edges()),
            21,
            "K_{3,7} should have 21 edges (3 * 7 connections).",
        )
        ...
        for dmr in range(3):  # DMR nodes are 0, 1, 2
            for gene in genes_k37:
                self.assertTrue(
                    graph_k37.has_edge(dmr, gene),
                    f"DMR {dmr} should be connected to {gene}.",
                )
