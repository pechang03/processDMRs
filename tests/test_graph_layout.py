...
class TestCalculateNodePositions(unittest.TestCase):
    ...
    def test_gene_split_bicliques(self):
        ...
        node_biclique_map = {
            1: [0],
            2: [0],  # DMRs in first biclique
            3: [1],
            4: [1],  # DMRs in second biclique
            5: [0],  # Gene only in first biclique
            6: [0, 1],  # Split gene (in both bicliques)
            7: [1],
            8: [1],  # Genes only in second biclique
        }
        ...
        # Split gene should be between its connected bicliques
        self.assertTrue(
            min(first_biclique_genes_y, second_biclique_genes_y)
            <= split_gene_y
            <= max(first_biclique_genes_y, second_biclique_genes_y),
