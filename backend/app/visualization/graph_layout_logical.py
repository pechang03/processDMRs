...
def calculate_node_positions(
    ...
    spacing = calculate_vertical_spacing(max_dmr_count, max_gene_count)
    ...
    for biclique_idx, (dmr_nodes, gene_nodes) in enumerate(bicliques):
        ...
        if nodes_positioned_this_biclique > 0:
            print(
                f"Biclique {biclique_idx}: Positioned {nodes_positioned_this_biclique} nodes"
            )

        biclique_height = calculate_biclique_height(
            dmr_nodes, gene_nodes, node_info.split_genes
        )
...
def calculate_vertical_spacing(
    max_dmr_count: int, max_gene_count: int, split_genes: Set[int] = None
) -> float:
    """
...
def position_biclique_nodes(
    ...
    gene_spacing = (
        biclique_height / (num_genes + 1) if num_genes > 0 else biclique_height
    )
