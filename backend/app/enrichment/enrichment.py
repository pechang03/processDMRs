...
from .ncbi_utils import fetch_ncbi_gene_ids
...
def get_biclique_genes(db: Session, biclique_id: int, timepoint_id: int) -> List[int]:
    ...
    try:
        ...
        query = select(Biclique.id, Biclique.gene_ids, Biclique.timepoint_id).where(
            Biclique.id == biclique_id, Biclique.timepoint_id == timepoint_id
        )
        result = db.execute(query).first()

        if not result:
            logger.error(
                f"No biclique found with id {biclique_id} at timepoint {timepoint_id}"
            )
            return []
        ...
        try:
            ...
            logger.info(
                f"Successfully parsed {len(gene_ids)} gene IDs from biclique {biclique_id}: {gene_ids}"
            )
            return gene_ids
...
def get_stored_biclique_enrichment(
    db: Session, biclique_id: int, timepoint_id: int
) -> Dict[str, Any]:
...
def save_biclique_enrichment_data(
    db: Session, biclique_id: int, timepoint_id: int, enrichment_data: Dict[str, Any]
):
    ...
    db.execute(
        insert(GOEnrichmentBiclique).values(
            ...
            p_value=min(
                [
                    term["p_value"]
                    for term in enrichment_data.get("top_processes", [{"p_value": 1.0}])
                ]
            ),
            enrichment_score=max(
                [
                    term["enrichment_score"]
                    for term in enrichment_data.get(
                        "top_processes", [{"enrichment_score": 0.0}]
                    )
                ]
            ),
            source="DAVID",
            biologicalProcessCount=len(enrichment_data.get("go_terms", {})),
            significantBiologicalProcesses=enrichment_data.get(
                "significant_processes", []
            ),
            topBiologicalProcess=enrichment_data.get("significant_processes", [""])[0],
            biologicalProcessAnnotationDetails=enrichment_data.get(
                "process_details", {}
            ),
        )
    )
...
def process_biclique_enrichment(
    db: Session, biclique_id: int, timepoint_id: int
) -> Dict[str, Any]:
    """
    ...
    """
    logger.info(
        f"Starting enrichment process for biclique {biclique_id} at timepoint {timepoint_id}"
    )
    update_process_status(
        db, "biclique", biclique_id, timepoint_id, EnrichmentStatus.INITIATED
    )
    ...
    update_process_status(
        db, "biclique", biclique_id, timepoint_id, EnrichmentStatus.FETCHING_GENES
    )
    ...
    update_process_status(
        db, "biclique", biclique_id, timepoint_id, EnrichmentStatus.FETCHING_NCBI_IDS
    )
    # Get NCBI IDs
    ...
    if not ncbi_id_mapping:
        ...
        update_process_status(
            ...
            biclique_id,
            timepoint_id,
            EnrichmentStatus.FETCHING_NCBI_IDS,
            info_msg,
        )
        try:
            ...
            if not ncbi_id_mapping:
                error_msg = (
                    f"Failed to fetch NCBI IDs for genes in biclique {biclique_id}"
                )
                logger.error(error_msg)
                update_process_status(
                    db,
    ...
    logger.info(
        f"Found {len(ncbi_id_mapping)} NCBI IDs for genes in biclique {biclique_id}"
    )
    ...
    update_process_status(
        ...
        biclique_id,
        timepoint_id,
        EnrichmentStatus.CALCULATING_ENRICHMENT,
    )
    ...
    try:
        ...
        if not enrichment_data:
            error_msg = (
                f"Failed to fetch enrichment data from DAVID for biclique {biclique_id}"
            )
            logger.error(error_msg)
            update_process_status(
                db,
    ...
    logger.info(
        f"Successfully fetched enrichment data from DAVID for biclique {biclique_id}"
    )

    update_process_status(
        db, "biclique", biclique_id, timepoint_id, EnrichmentStatus.SAVING_RESULTS
    )
    ...
    try:
        ...
        update_process_status(
            db, "biclique", biclique_id, timepoint_id, EnrichmentStatus.COMPLETED
        )
    except Exception as e:
        error_msg = (
            f"Failed to save enrichment data for biclique {biclique_id}: {str(e)}"
        )
        logger.error(error_msg)
        update_process_status(
            db,
...
def get_adjacent_genes_for_dmr(
    db: Session, dmr_id: int, timepoint_id: int
) -> List[int]:
    """Get all genes adjacent to a DMR"""
    query = select(EdgeDetails.gene_id).where(
        EdgeDetails.dmr_id == dmr_id, EdgeDetails.timepoint_id == timepoint_id
    )
    result = db.execute(query)
...
def get_stored_enrichment(
    db: Session, dmr_id: int, timepoint_id: int
) -> Dict[str, Any]:
    """Get stored GO enrichment data if it exists"""
    query = select(GOEnrichmentDMR).where(
        GOEnrichmentDMR.dmr_id == dmr_id, GOEnrichmentDMR.timepoint_id == timepoint_id
    )
    result = db.execute(query).first()
    if result:
        # Get top processes
...
def fetch_go_enrichment(ncbi_ids: List[str], organism: str = "human") -> Dict[str, Any]:
    ...
    try:
        response = requests.post(
            api_url, json={"genes": ncbi_ids, "organism": organism}
        )
        response.raise_for_status()
...
def save_enrichment_data(
    db: Session, dmr_id: int, timepoint_id: int, enrichment_data: Dict[str, Any]
):
