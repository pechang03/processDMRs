...
def get_bicliques_summary(bicliques_result: Dict, original_graph: nx.Graph) -> Dict:
    ...
    summary = {
        ...
        "uncovered_edges_sample": bicliques_result.get("debug", {}).get(
            "uncovered_edges", []
        ),
...
def print_bicliques_summary(bicliques_result: Dict, original_graph: nx.Graph) -> None:
    ...
    print(f"\nEdge Coverage:")
    print(
        f"Single coverage: {edge_cov['single_coverage']} edges ({edge_cov['single_percentage']:.1%})"
    )
    print(
        f"Multiple coverage: {edge_cov['multiple_coverage']} edges ({edge_cov['multiple_percentage']:.1%})"
    )
    print(
        f"Uncovered: {edge_cov['uncovered']} edges ({edge_cov['uncovered_percentage']:.1%})"
    )
    ...
    if edge_cov["uncovered"] > 0:
        ...
        for edge in summary["uncovered_edges_sample"]:
            print(f"  {edge}")
        print(f"Total nodes involved in uncovered edges: {summary['uncovered_nodes']}")
...
def create_node_labels_and_metadata_depricated(
    df: pd.DataFrame,
...
def create_statistics_summary(
    bicliques_result: Dict, edge_classification: Dict[str, List[EdgeInfo]] = None
) -> Dict:
    """
    ...
    if edge_classification:
        ...
        bridge_counts = {"bridge_false_positive": 0, "potential_true_bridge": 0}
        if "bridge_edges" in edge_classification:
            bridge_counts["bridge_false_positive"] = len(
                edge_classification["bridge_edges"]["false_positives"]
            )
            bridge_counts["potential_true_bridge"] = len(
                edge_classification["bridge_edges"]["potential_true_bridges"]
            )
    ...
    summary = {
        "coverage": {
            ...
            "edges": {
                ...
                "percentages": {
                    label: count / sum(edge_counts.values())
        ...
        "validation": {
            "false_positives_match": edge_counts["false_positive"]
            == header_stats.get("Nb deletions", 0),
            "false_negatives_match": edge_counts["false_negative"]
            == header_stats.get("Nb additions", 0),
        },
        "size_distribution": calculate_size_distribution(
            bicliques_result.get("bicliques", [])
        ),
        "classifications": classify_biclique_types(
            bicliques_result.get("bicliques", [])
        ),
    }
...
def create_node_labels_and_metadata(
    ...
    for _, row in df.iterrows():
        ...
        if dmr_id < 5:
            ...
            print(
                f"  Description: {row['Gene_Description'] if 'Gene_Description' in df.columns else 'N/A'}"
            )
