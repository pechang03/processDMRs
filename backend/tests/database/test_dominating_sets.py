...
class TestDominatingSetStorage(unittest.TestCase):
    def setUp(self):
        ...
        self.timepoint = Timepoint(
            name="test_timepoint", sheet_name="test_sheet", description="Test timepoint"
        )
    ...
    def test_store_dominating_set(self):
        ...
        # Verify storage
        stored_entries = (
            self.session.query(DominatingSet)
            .filter_by(timepoint_id=self.timepoint.id)
            .all()
        )
        ...
        for entry in stored_entries:
            ...
            self.assertEqual(
                entry.dominated_gene_count, self.dominated_counts[entry.dmr_id]
            )
