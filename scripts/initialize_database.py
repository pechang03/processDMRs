...
def populate_timepoints(session: Session):
    ...
    timepoints = ["DSS1", "P21-P28_TSS", "P21-P40_TSS", "P21-P60_TSS", "P21-P180_TSS", 
                  "TP28-TP180_TSS", "TP40-TP180_TSS", "TP60-TP180_TSS"]
...
def populate_bicliques(session: Session, bicliques_result: dict, timepoint_id: int):
    ...
    for biclique in bicliques_result['bicliques']:
        ...
        operations.insert_biclique(session, timepoint_id, None, list(dmr_ids), list(gene_ids))
    ...
    for component in bicliques_result.get('components', []):
        if isinstance(component, dict):
            ...
            for biclique in component.get('raw_bicliques', []):
                biclique_id = operations.insert_biclique(session, timepoint_id, comp_id, list(biclique[0]), list(biclique[1]))
    ...
    for entity_type, entity_data in metadata.items():
        for entity_id, entity_metadata in entity_data.items():
            for key, value in entity_metadata.items():
                operations.insert_metadata(session, entity_type, entity_id, key, str(value))
        ...
        with Session(engine) as session:
            ...
            for timepoint in timepoints:
                ...
                if timepoint.name == "DSS1":
                    ...
                    df = pd.read_excel(constants.DSS_PAIRWISE_FILE, sheet_name=timepoint.name)
                
                # Populate DMRs
                populate_dmrs(session, df, timepoint.id)
                
                # Process bicliques
                biclique_file = constants.BIPARTITE_GRAPH_TEMPLATE.format(timepoint.name)
                if os.path.exists(biclique_file):
                    ...
                    populate_relationships(session, bicliques_result.get('relationships', []))
                else:
