...
def _add_biclique_details(
    bicliques: List, df: pd.DataFrame, gene_id_mapping: Dict, node_biclique_map: Dict[int, List[int]]
) -> Dict:
    ...
    for idx, (dmr_nodes, gene_nodes) in enumerate(bicliques):
        detailed_info[f"biclique_{idx+1}_details"] = {
            ...
            "split_genes": [
                gene for gene in gene_nodes if len(node_biclique_map.get(gene, [])) > 1
            ],
        }
