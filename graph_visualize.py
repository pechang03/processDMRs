...
def create_dmr_table(dmr_metadata: Dict[str, Dict]) -> go.Table:
    ...
    rows = [
        [dmr, metadata.get("area", "N/A"), ", ".join(map(str, metadata.get("bicliques", [])))]
...
def create_gene_table(gene_metadata: Dict[str, Dict], gene_id_mapping: Dict[str, int], node_biclique_map: Dict[int, List[int]]) -> go.Table:
    """Create a Plotly table for gene metadata."""
    headers = ["Gene", "Description", "Bicliques"]
    rows = [
        [gene, metadata.get("description", "N/A"), ", ".join(map(str, node_biclique_map.get(gene_id_mapping[gene], [])))]
